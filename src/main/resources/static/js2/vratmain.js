...
$(document).ready(function() {
	...
	$.ajax({
		...
		error : function(jqXHR, textStatus, errorThrown) {
			console.log("vrat main:Could not get vrat count");
//			window.location.href='/errorpage';
	...
	$.ajax({
		...
		error : function(jqXHR, textStatus, errorThrown) {
			console.log("vrat main:Could not get vrat cat");
//			window.location.href='/errorpage';
		},
...
function setFilter(cats) {
	...
	for(var i in cats){
		var cat = cats[i];
		if((cat.type && cat.type.toUpperCase() == 'VRAT') || cat.name.toUpperCase() == "ALL" ) {
...
function changeFilter(filterId, itemId, itemName) {
	...
	$.ajax({
		url : "/vratCountByFilter/"+currCatId+"/"+currItemPageCount,
		type : "GET",
		dataType : 'json',
		success : function(resultData) {
			//here is your json.
			// process it
			console.log("vrat count by cat is called "+resultData);
			total = resultData;
			
			showPage(0);
		},
		error : function(jqXHR, textStatus, errorThrown) {
			console.log("vrat main:Could not get vrat count by cat");
//			window.location.href='/errorpage';
		},
...
function showPage(pageId) {
	...
	$.ajax({
		url : "/vrats/"+pageId+"/"+catId+"/"+itemPage,
		type : "GET",
		async : 'false',
		dataType : 'json',
		success : function(resultData) {
