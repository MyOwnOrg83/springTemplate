...
$(document).ready(function() {
	...
	$.ajax({
		...
		success : function(resultData) {
			//here is your json.
			// process it
			console.log("chalisa main is called"+resultData);
			total = resultData;
		...
		error : function(jqXHR, textStatus, errorThrown) {
			console.log("chalisa main:Could not get count");
...
function parseData(results) {
	...
	for(var i in content){
        ...
        html += '<a href="chalisa-single.html?key='+result.id+'">';
...
//        html += '<h4>About</h4>';
        ...
        html += '<a class="btn btn-primary" href="chalisa-single.html?key='+result.id+'">View More <span class="glyphicon glyphicon-chevron-right"></span></a>';
        html += '</div>';
        html += '</div>';
...
function showPage(pageId) {
	$.ajax({
		...
		success : function(resultData) {
			//here is your json.
			// process it
			console.log("show page chalisa main is called"+pageId);
			parseData(resultData);
			parsePaging(pageId, total);
		},
		error : function(jqXHR, textStatus, errorThrown) {
			console.log("show page chalisa main:error is called");
			window.location.href='/errorpage';
		},
		timeout : 120000,
	});
